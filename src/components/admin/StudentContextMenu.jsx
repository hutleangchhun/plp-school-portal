...
const StudentContextMenu = ({ children, student, onResetPassword, onDelete, onToggleActiveStatus, onDownloadQRCode }) => {
  const { t } = useLanguage();
  ...
  const handleCopyUsername = () => {
    ...
    if (navigator.clipboard && navigator.clipboard.writeText) {
      navigator.clipboard.writeText(student.username).then(
        () => {
          showSuccess(t('usernameCopied', 'Username copied to clipboard'), 2000);
        },
        (err) => {
          console.error('Clipboard API error:', err);
          fallbackCopyToClipboard(student.username);
        }
      );
    } else {
