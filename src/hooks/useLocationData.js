...
export const useLocationData = (initialValues = {}) => {
  ...
  const setInitialValues = useCallback((values) => {
    if (values.provinceId) setSelectedProvince(values.provinceId.toString());
    if (values.districtId) setSelectedDistrict(values.districtId.toString());
    if (values.communeId) setSelectedCommune(values.communeId.toString());
    if (values.villageId) setSelectedVillage(values.villageId.toString());
  }, []);
